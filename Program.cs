        ...
        try
        {
            ...
            for (int p = 0; p < qPontosCarga; p++)
            {
                for (int i = 0; i < qViagens; i++)
                {
                    ...
                    {
                        cplex.AddLe(cplex.Sum(cplex.Prod(M, cplex.Diff(1, x[i][j][p])), tfp[j]),
            ...
            for (int i = 0; i < qViagens; i++)
            {
                ...
                cplex.AddGe(atrp[i], cplex.Diff(hcc[i], 
                    cplex.Diff(tfp[i],
                        cplex.Sum(
                            cplex.ScalProd(vp[i], dv[i]),
                            cplex.ScalProd(vp[i], dp[i])))));
                cplex.AddLe(atrp[i], cplex.Diff(hcc[i],
                    cplex.Diff(tfp[i],
                        cplex.Sum(
                            cplex.ScalProd(vp[i], dv[i]),
                            cplex.ScalProd(vp[i], dp[i])))));

                cplex.AddGe(avnp[i], cplex.Diff(
                    cplex.Diff(tfp[i],
                        cplex.Sum(
                            cplex.ScalProd(vp[i], dv[i]),
                            cplex.ScalProd(vp[i], dp[i]))),
                    hcc[i]));
                cplex.AddLe(avnp[i], cplex.Diff(
                    cplex.Diff(tfp[i],
                        cplex.Sum(
                            cplex.ScalProd(vp[i], dv[i]),
                            cplex.ScalProd(vp[i], dp[i]))),
